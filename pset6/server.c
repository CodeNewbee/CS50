...
char* indexes(const char* path)
{
    ...
    //create path to index.php file
    char* new_path = malloc(strlen(path) + strlen(php_path) + 1);
    strcat(new_path, path);
    strcat(new_path, php_path);
    
    //if index.php file exsists, return path
    ...
    //create path to index.html file
    new_path = malloc(strlen(path) + strlen(html_path) + 1);
    strcat(new_path, path);
    strcat(new_path, html_path);
    
    //if index.html file exsists return path
    if(access(new_path, F_OK) != -1)
    {
        return new_path;
    }

    //if neither exsist, return NULL
    free(new_path);
    return NULL;
}
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    //if method is not GET respond with error 405 method not allowed
    ...
    //if request-target doesn't start with / respond with error 501 not implemented
    if(line[4] != '/')
    {
        error(501);
        return false;
    ...
    //if request-line contains " respond with erro 400 bad request
    if(strchr(line, '"') != NULL)
    {
        error(400);
        return false;
    ...
    //if HTTP version is not 1.1 respond with error 505 HTTP version not supported
    ...
    //if line contains two spaces respond with error 400 bad request
    if(strstr(line, "  ") != NULL)
    {
        error(400);
        return false;
    ...
    //find index where absolute path ends
    int index = 0;
    for(int i = 4; i < LimitRequestLine + 1; i++)
    {
        if(line[i] == ' ' || line[i] == '?')
        {
            index = i;
    ...
    //store absolute path
    strncpy(abs_path, line + 4, index - 4);
    abs_path[index-4] = '\0';
    ...
    {
        index++;
        for(int i = 0; line[index] != 'H'; i++)
        {
            query[i] = (line[index] == ' ') ? '\0': line[index];
            index++;
        }
        return true;
    }
